...
class FFragment():
   ...
   def MakeEmbeddedSystem(self, DmetParams):
      ...
      if not DmetParams.UseInt2e:
         ...
         Int2e_Ji = np.zeros((nOrb, nOrb))
         # allow for unpacked 
         # passing of integrals in the form (ij|kl) from fragments to quantum_system.py
         # in function FSystem
         for (i,iSite) in enumerate(self.Sites):
            ...
            for (j, jSite) in enumerate(self.Sites):
               if (j != i):
                 Int2e_Ji[i,j] = fs.Model.GetJi(fs.UnitCell[iSite],fs.UnitCell[jSite])
         Uprime = np.zeros_like(Int2e_Ji, Int2e_Ui.dtype)
         if Int2e_Ji.any != 0.0:
           for (i,iSite) in enumerate(self.Sites):
              if (Int2e_Ui[i]!=0.0): U = Int2e_Ui[i]
              for (j, jSite) in enumerate(self.Sites):
                 if (j != i):
                    Uprime[i,j] =  U * (Int2e_Ji[i,j]!=0.0) - 2.0 * Int2e_Ji[i,j] 
                    Int2e_4ix[i,j,i,j] = Int2e_Ji[i,j]
                    Int2e_4ix[i,i,j,j] =  Uprime[i,j]
         ...
         # transform U,J from full system basis into embedded basis
         Int2e_Ui_SuperCell = fs.SuperCell.MakeUiMatrix(fs.SuperCell.Sites)
         Int2e_Ji_SuperCell = fs.SuperCell.MakeJiMatrix(fs.SuperCell.Sites,fs.SuperCell.Sites)
         Uprime_SuperCell = np.zeros_like(Int2e_Ji_SuperCell, Int2e_Ui_SuperCell.dtype)
         if Int2e_Ji_SuperCell.any != 0.0:
           for (i,iSite) in enumerate(self.Sites):
              if (Int2e_Ui_SuperCell[i]!=0.0): U = Int2e_Ui_SuperCell[i]
              for (j, jSite) in enumerate(self.Sites):
                 if (j != i):
                    Uprime_SuperCell[i,j] =  U * (Int2e_Ji_SuperCell[i,j]!=0.0) - 2.0 * Int2e_Ji_SuperCell[i,j] 
         #if (Int2e_Ji_SuperCell.any != 0.0): 
         #   raise Exception("MakeInt2e is not implemented for J-coupling")
         #transform to embedding basis 
         # This function is much too slow for Ji!=0
         Int2e_4ix = MakeInt2e(Int2e_Ui_SuperCell,Int2e_Ji_SuperCell, Uprime_SuperCell,EmbBasis, EmbBasis)
         Int2e_Ui = None
         Int2e_Ji = None
      ...
      if self.FullSystem.HaveFock:
        CoreFock = EmbFock - self.MakeFock(EmbRdm, CoreH, Int2e_Ui, Int2e_Ji, Int2e_4ix, OrbType=fs.WfDecl.OrbType)[0]
      else:
        CoreFock = 0.*EmbFock
   ...
   def MakeFock(self, Rdm, CoreH, Int2e_Ui, Int2e_Ji, Int2e_4ix=None, Orb=None, OrbType=None):
      if OrbType is None:
         OrbType = self.WfDecl.OrbType
      if Int2e_Ui is not None:
         if Int2e_Ji is not None:
            print "Note: The J-coupling term is not considered in the Fock-operator construction."
            # see meanfield.py.
         if OrbType == "RHF":
            #old Hubbard code
...
def MakeInt2e(Ui, Ji, Uprime, EmbBasisIJ, EmbBasisKL):
    ...
    Tmp   = np.zeros( (nOrb,nOrb,nOrb,nOrb) )
    def outer4(a,b,c,d):
       na = np.newaxis
       return ((a[:,na] * b[na,:])[:,:,na] * c[na,:])[:,:,:,na] * d[na,:]
    for iSite in range(EmbBasisIJ.shape[0]):
       u = EmbBasisIJ[iSite,:]
       Int2e[:,:,:,:] += Ui[iSite]*outer4(u,u,u,u)
       if (Ji.any != 0.0):
         for jSite in range(iSite):
            v = EmbBasisIJ[jSite,:]
            Int2e[:,:,:,:] += Ji[iSite,jSite] * outer4(u,u,v,v)+Uprime[iSite,jSite] * outer4(u,v,u,v)
    return Int2e

def MakeFockX1(Rdm, CoreH, Int2e_Ui, Int2e_Ji, Int2e_4ix=None, Orb=None, OrbType="RHF"):
   if Int2e_Ji is not None:
      print "Note: J-coupling is not taken into account in the Fock-operator construction."
   if Int2e_Ui is not None:
      # see meanfield.py.
      if OrbType == "RHF":
         Rho = np.real(np.diag(Rdm)) # <- always real
...
class FDmetContext:
   ...
   def CheckFragmentation(self, Log):
      ...
      for Site in range(MaxSite):
         ...
         elif abs(SiteOccurence[Site] - 1.) > 1e-8:
